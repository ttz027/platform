 ...
 * @Author ztt
 * @Description
 * @Date 2020/9/30 0030 下午 2:09
 */
public class SignUtil {

//    private Jedis jedis = new Jedis();
    private static Jedis jedis = SpringContextHolder.getBean(Jedis.class);
     ...
     * @return 之前的签到状态
     */
    public static boolean doSign(int uid, LocalDate date) {
     ...
     * @return 当前的签到状态
     */
    public static boolean checkSign(int uid, LocalDate date) {
        int offset = date.getDayOfMonth() - 1;
     ...
     * @return 当前的签到次数
     */
    public static long getSignCount(int uid, LocalDate date) {
     ...
     * @return 当月连续签到次数
     */
    public static long getContinuousSignCount(int uid, LocalDate date) {
        int signCount = 0;
     ...
     * @return 首次签到日期
     */
    public static LocalDate getFirstSignDate(int uid, LocalDate date) {
     ...
     */
    public static Map<String, Boolean> getSignInfo(int uid, LocalDate date) {
    ...
    public static void main(String[] args) {
        ...
        { // doSign
            boolean signed = SignUtil.doSign(1000, today);
            if (signed) {
        ...
        { // checkSign
            boolean signed = SignUtil.checkSign(1000, today);
            if (signed) {
                System.out.println("您已签到：" + formatDate(today, "yyyy-MM-dd"));
            } else {
        ...
        { // getSignCount
            long count = SignUtil.getSignCount(1000, today);
            System.out.println("本月签到次数：" + count);
        }

        { // getContinuousSignCount
            long count = SignUtil.getContinuousSignCount(1000, today);
            System.out.println("连续签到次数：" + count);
        }

        { // getFirstSignDate
            LocalDate date = SignUtil.getFirstSignDate(1000, today);
